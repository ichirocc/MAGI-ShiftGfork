...
object UnimprovableConstraints {
    ...
    private fun logReport(findings: List<Finding>, exclude: Set<String>, provenHard: Int) {
        if (findings.isEmpty()) {
            println("[MAGI_UNIMPROVABLE] none")
            return
        }
        for (f in findings) {
            val kind = if (f.hard) "HARD" else "SOFT"
            println("[MAGI_UNIMPROVABLE] $kind family=${f.family} count=${f.count} ${f.reason}")
        }
        println("[MAGI_UNIMPROVABLE] summary provenHardUnits=$provenHard exclude=$exclude")
    }
